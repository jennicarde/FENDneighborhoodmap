    ...
    self.getYelp = function(yelpBusinessId, marker) {
        ...
        var settings = {
            ...
            success: (function(result, textStatus, jqXHR) {
                ...
                // var phone;
                // if ( yelpResult.displayPhone) {
                //       phone = yelpResult.displayPhone ;
                // }  else  phone  = "no phone available";

                var name = yelpResult.name ? yelpResult.name : "no name available";
                var categories = yelpResult.categories ? yelpResult.categories : "no categories available";
                var rating = yelpResult.rating ? yelpResult.rating : "no rating available";
                var address = yelpResult.location.address ? yelpResult.location.address : "no address available";
                var phone  = yelpResult.display_phone ?  yelpResult.display_phone : "no phone available";
                var snippet = yelpResult.snippet_text ? yelpResult.snippet_text : "no information available";

                var yelpContentStr = '';

                yelpContentStr += '<div class ="info">' +
                    '<h2 class="name">' + name + '</h2>' +
                    '<p class="categories">' + "<b>Categories: </b>" + categories[0][0].toString() + '</p>' +
                    '<p class="rating">' + "<b>Rating: </b>" + rating + '</p>' +
                    '<p class="location">' + "<b>Address: </b>" + address.toString() + '</p>' +
                    '<p class="phone">' + "<b>Phone: </b>" + phone + '</p>' +
                    '<p class="snippet">' + "<b>About: </b>" + snippet + '</p>' +
